...
par(mfrow=c(2,1))
str(population)
hist(population, xlim=c(0,4000))
hist(samp, xlim=c(0,4000))
```
...
summary(samp)
```
*The distribution of the sample looks like an unimodal normal distribution. I would consider the median value of the sample to be typical size of the houses. This value changes everytime we re-sample the population.*
2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
*I will expect most student's distributions to be different than mine as they sample 60 values from 2930 possible values.*
...
*The confidence interval (1361.823, 1599.810) calculated previously in this lab does indeed captures the mean value (1499.69) of the population.*
...
population mean.
*95% of the calculated intervals should capture the true population mean. *
...
*From the plot below we see that 49 (or 98%) of the calculated confidence intervals contained the true population mean. This proportion is higher than the chosen confidence value of 95%. This is OK because everytime we choose a sample population we get a new subset of values. 95% or more of those will contain the true population mean.*
...
*If we choose a confidence level of 90%, the critical value or z-score is 1.645*
...
*Constructing the confidence intervals for 90% confidence level.*

```{r  eval=TRUE}
lower_vector.90 <- samp_mean - 1.645 * samp_sd / sqrt(n) 
upper_vector.90 <- samp_mean + 1.645 * samp_sd / sqrt(n)
head(lower_vector.90)
head(upper_vector.90)
```
...
*In the calculated range we see that with a chosen confidence interval o 90%, 48 of our interval ranges (96%) contain the true mean of the population.*
