...
```{r}
par(mfrow=c(1,2))
hist(fdims$hgt, breaks = 20, main = "Women's Heights", xlab="Height", col = "red",prob=TRUE)
curve(dnorm(x, mean=mean(fdims$hgt), sd=sd(fdims$hgt)), col="black", lwd=2, add=TRUE, yaxt="n")
hist(mdims$hgt, breaks = 20, main = "Men's Heights", xlab="Height", col = "blue",prob=TRUE)
curve(dnorm(x, mean=mean(mdims$hgt), sd=sd(mdims$hgt)), col="black", lwd=2, add=TRUE, yaxt="n")
```
**Summary of female height entries**
```{r}
summary(fdims$hgt)
```
**Summary of male height entries**
```{r}
summary(mdims$hgt)
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
**The normal distribution does indeed look like a good fitting curve to the female heigh data.**
...
**The normal probability plot for `fdims$hgt` look similar to the simulated plots. However, the probability plot for `fdims$hgt` has a staircase shape. Could be due to women tending to round their age when reporting. Therefore, the height data does not have a continous range.** 
...
qqnormsim(fdims$wgt)
```
**The reported data of female weights does not appear to come from a normal distribution.**
...
**What is the probability that a randomly chosen young adult female is taller than +1 standard deviation from the mean?**
```{r}
1 - pnorm(q = (fhgtmean+fhgtsd), mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt > (fhgtmean+fhgtsd)) / length(fdims$hgt)
```

**What is the probability that a randomly chosen young adult female weights less than 50 kilos.**
```{r}
summary(fdims$wgt)
...
__Height had a closer agreement between both methods.__
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter __C_.

    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter __B_.
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter __A_.

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
    
**The measurements may be rounded up/down when given leading to a stepped distribution of measurements.**
...
qqline(fdims$kne.di)
```
**Based on the above normal probability plot, I can say that the variable is right or positively skewed.** 

```{r}
hist(fdims$kne.di, breaks = 20, xlim=c((mean(fdims$kne.di)-6),(mean(fdims$kne.di)+6)), main = "Women's Knee Diameter", xlab="Knee Diameter", col = "red",prob=TRUE)
curve(dnorm(x, mean=mean(fdims$kne.di), sd=sd(fdims$kne.di)), col="black", lwd=2, add=TRUE, yaxt="n")
```
