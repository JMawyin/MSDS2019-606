...
*The distribution looks unimodal and positive or right skewed.*
...
*The distribution of the sample looks similar to the distribution of the complete population. Still looks unimodal and right skewed.*
...
```{r}
samp.100 <- sample(area, 100)
samp.1000 <- sample(area, 1000)
mean(samp.100); mean(samp.1000)
```

*The larger the sample, the closer it gets to the true mean value of the population.*
...
*The sample_means50 contains 5000 samples. The distribution has the following properties:*

Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
1269    1450    1498    1499    1545    1770 

*The distribution will be dependent on the specific samples taken for the set. However, the larger the set, the closer it will be to the true value of the whole population.*
...
*`sample_means_small` will contains 100 elements. Each element contains the mean of a sample of random 50 area observations from the complete set.*
...
*The larger the sample size, the center (mean) of the distribution does not appear to change much. However, is very clear that the spread decreases with a larger sample.*
...
```{r}
library(magrittr)
samp.price.50 <- sample(price, 50)
samp.price.50.mean <- mean(samp.price.50) %>% round(0)
cat("My best point estimate of the population mean is", samp.price.50.mean, "residents.")
```
...
```{r}
hist(sample_means50, breaks = 20, xlim = c(1300,1800))
sample_means50.summary <- summary(sample_means50)
sample_means50.summary
```

*Just looking at the histogram of sample_means50, I would estimate the sample mean to be around 1490 people - population wise.*

```{r}
cat("The calculated population mean is: 1499 people")
```
...
```{r}
hist(sample_means150, breaks = 20, xlim = c(1300,1800))
sample_means150.summary <- summary(sample_means150)
sample_means150.summary
```

*The distribution of "sample_means150" looks nomarl and unimodal. Compared to "sample_means50", "sample_means150" has the same mean value (~1490) but a smaller spread.*
...
*"sample_means150" has the same mean value (~1490) but a smaller spread. The greater the sample size, the closer the sample mean value will be to the true mean value.*
